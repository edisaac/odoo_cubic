...
class stock_planning_createlines(osv.osv_memory):
    ...
    _columns = {
        ...
        'warehouse_id': fields.many2one('stock.warehouse' , 'Warehouse', required=False, help = 'Warehouse which planning will concern.'),
    ...
    def create_planning(self,cr, uid, ids, context=None):
        ...
        for f in self.browse(cr, uid, ids, context=context):
            warehouse_ids = []
            if f.warehouse_id:
                warehouse_ids = [f.warehouse_id.id]
            else:
                company_id = user_obj.browse(cr, uid, uid).company_id.id
                warehouse_ids = warehouse_obj.search(cr,uid,[('company_id','=',company_id)])
                
            for wh in warehouse_obj.browse(cr,uid,warehouse_ids,context=context):
            
                if f.forecasted_products:
                    cr.execute("SELECT product_id \
                                    FROM stock_sale_forecast \
                                    WHERE (period_id = %s) AND (warehouse_id = %s) and product_qty>0 and state='validated'", (f.period_id.id, wh.id))
                    ...
                    if len(planning_obj.search(cr, uid, [('product_id','=',p.id),
                        ...
                        cr.execute("SELECT period.date_stop, planning.product_uom, planning.planned_outgoing, planning.to_procure, \
                                    ...
                                    ORDER BY period.date_stop DESC",
                                        (uid, uid, wh.id, p.id, f.period_id.date_stop) )
                        ret=cr.fetchone()
                        if ret:
                            prod_uom = ret[1]
                            planned_out = ret[2]
                            ...
                            to_procure = False
                            stock_only = True
                            procure_to_stock = True
                            confirmed_forecasts_only = False
                            #supply_warehouse_id = False
                            stock_supply_location = True
                        prod_uos_categ = False
                        if f.forecasted_products:
                            cr.execute("SELECT product_qty,product_uom \
                                    FROM stock_sale_forecast \
                                    WHERE (period_id = %s) AND (warehouse_id = %s) and product_id = %s and state='validated'", (f.period_id.id, wh.id, p.id))
                            ret = cr.fetchone()
                            if ret:
                                to_procure =  ret[0]
                                prod_uom =   ret[1]
                        #netsvc.Logger().notifyChannel("create_planning", netsvc.LOG_INFO, "to_procure: %s - (period_id = %s) AND (warehouse_id = %s) and product_id = %s"%(to_procure,f.period_id.id, wh.id, p.id))
                        supply_warehouse_id = wh.supply_warehouse_id.id
                        if p.uos_id:
        ...
        planning_obj.calculate_planning(cr, uid, planning_lines, context)
