...
class account_voucher(osv.osv):
    ...
    def _get_journal(self, cr, uid, context=None):
        ...
        if context.get('invoice_id', False):
            ...
            return journal_id and journal_id[0] or context.get('journal_id', False)
    ...
    def _get_tax(self, cr, uid, context=None):
        ...
        if account_id and account_id.tax_ids:
            #YT 2/12/2011
            # Fix me, only support one payment tax
            #tax_id = account_id.tax_ids[0].id
            tax_id = tax_id_pay = False
            for tax in account_id.tax_ids:
                if self.pool.get('account.tax').fields_get(cr,uid,['payment'],context=context) and tax.payment:
                    tax_id_pay = tax.id
                else:
                    tax_id = tax.id
            if context.get('type', False) in ('payment', 'receipt', False):
                tax_id = tax_id_pay
...
#        'currency_id': fields.related('journal_id','currency', type='many2one', relation='res.currency', string='Currency', store=True, readonly=True, states={'draft':[('readonly',False)]}),
    ...
    def action_move_line_create(self, cr, uid, ids, context=None):
        ...
        for inv in self.browse(cr, uid, ids, context=context):
            ...
            #YT 15/12/2011 tax cash module
            if self.pool.get('account.tax').fields_get(cr,uid,['payment'],context=context) and not transfer:
                move_line1 = move_line_pool.browse(cr, uid, move_line1_id, context=context)
                for tax in move_line1.account_id.tax_ids:
                    if not tax.payment: continue
                    amnt = move_line1.credit or move_line1.debit
                    taxes = tax_obj.compute_all(cr, uid, [tax], amnt, 1)
                    ratio = taxes['total'] and taxes['total_included'] / taxes['total'] - 1 or 0.0
                    move_line = {
                        'name': "%s"%(tax.name) ,
                        ...
                        'debit': amnt * ratio,
                        'credit': 0.0,
                        'amount_currency': abs(move_line1.amount_currency) * ratio,
                        'currency_id': move_line1.currency_id and move_line1.currency_id.id or False,
                    }
                    move_line_pool.create(cr, uid, move_line)
                    move_line = {
                        'name': "%s"%(tax.name) ,
                        'account_id': move_line1.account_id.id,
                        'move_id': move_id,
                        #'partner_id': inv.partner_id.id,
                        'date': inv.date,
                        'debit': 0.0,
                        'credit': amnt * ratio,
                        'amount_currency': abs(move_line1.amount_currency) * ratio * -1,
                        'currency_id': move_line1.currency_id and move_line1.currency_id.id or False,
                    }
                    move_line_pool.create(cr, uid, move_line)
