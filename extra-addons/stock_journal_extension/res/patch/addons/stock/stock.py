...
class stock_picking(osv.osv):
    ...
    def create(self, cr, user, vals, context=None):
        ...
        netsvc.Logger().notifyChannel("create", netsvc.LOG_INFO, "vals:%s - context:%s"%(vals,context))
        new_id = super(stock_picking, self).create(cr, user, vals, context)
        return new_id

    _columns = {
        'name': fields.char('Reference', size=64, select=True, readonly=True, states={'assigned':[('readonly',False)]}),
        ...
        'type': fields.selection([('out', 'Sending Goods'), ('in', 'Getting Goods'), ('internal', 'Internal')], 'Shipping Type', required=True, select=True, help="Shipping type specify, goods coming in or going out.", states={'done':[('readonly',True)],'cancel':[('readonly',True)]}),
        'note': fields.text('Notes'),
        'stock_journal_id': fields.many2one('stock.journal','Stock Journal', states={'done':[('readonly',True)],'cancel':[('readonly',True)]}, select=True),
        ...
        'move_type': fields.selection([('direct', 'Partial Delivery'), ('one', 'All at once')], 'Delivery Method', required=True, help="It specifies goods to be delivered all at once or by direct delivery", states={'done':[('readonly',True)],'cancel':[('readonly',True)]}),
        ...
        'address_id': fields.many2one('res.partner.address', 'Address', help="Address of partner", states={'done':[('readonly',True)],'cancel':[('readonly',True)]}),
