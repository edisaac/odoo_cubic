...
class delivery_route(osv.osv):
    ...
    _columns = {
            'name': fields.char('Reference', size=64, required=True, select=True,states={'cancel':[('readonly',True)],
                                                                                         'done':[('readonly',True)],}),
            'date': fields.date('Date', required=True, select=True,states={'cancel':[('readonly',True)],'done':[('readonly',True)],}),
            'time_id': fields.many2one('delivery.time','Delivery Time', select=True, states={'cancel':[('readonly',True)],'done':[('readonly',True)],}),
            'driver_id': fields.many2one('delivery.driver','Delivery Driver', required=True, states={'cancel':[('readonly',True)],'done':[('readonly',True)],}),
            ...
            'line_ids': fields.one2many('delivery.route.line','route_id','Lines',required=True, states={'cancel':[('readonly',True)],'done':[('readonly',True)],}),
            'departure_date': fields.datetime('Departure Date', states={'cancel':[('readonly',True)],'done':[('readonly',True)],}),
            'arrive_date': fields.datetime('Arrive Date', states={'cancel':[('readonly',True)],'done':[('readonly',True)],}),
        }
    _defaults = {
            'state': 'draft',
            'name': '/'
...
class delivery_route_line(osv.osv):
    ...
    def _get_route_line_from_route(self, cr, uid, ids, context=None):
        res = {}
        for route in self.pool.get('delivery.route').browse(cr,uid,ids,context=context):
            for line in route.line_ids:
                res[line.id] = True
        return res.keys()
    
    def _route_date(self, cr, uid, ids, field_name, arg, context):
        if context is None: context = {}
        res = {}
        for line in self.browse(cr,uid,ids,context=context):
            if line.route_id:
                res[line.id] = line.route_id.date
            else:
                res[line.id] = False
        return res
    
    _columns = {
            'sequence': fields.integer('Sequence',states={'cancel':[('readonly',True)]}),
            'route_id': fields.many2one('delivery.route','Delivery Route', required=True, ondelete="cascade",select=True,readonly=True,states={'draft':[('readonly',False)]}),
            'route_date': fields.function(_route_date,type='date',method=True,string='Route Date',readonly=True,
                                            store={'delivery.route':(_get_route_line_from_route,['date'],10)}),
            'picking_id': fields.many2one('stock.picking','Picking', required=True,select=True,readonly=True,states={'draft':[('readonly',False)]}),
            'address_id': fields.related('picking_id','address_id',type='many2one',relation='res.partner.address',select=True,string='Delivery Address',readonly=True),
            'picking_delivered': fields.related('picking_id','delivered',type='boolean',string='Picking Delivered',readonly=True),
            'picking_state': fields.related('picking_id','state',type='char',string='Picking State',readonly=True),
            'state': fields.selection([
                                ('draft','Draft'),
                                ('delivered','Delivered'),
                                ('received','Received'),
            ...
            'note': fields.text('Notes',states={'cancel':[('readonly',True)]}),
        }
    _defaults = {
            'state': 'draft',
        }
    ...
    def action_received(self, cr, uid, ids, context=None):
        for line in self.browse(cr,uid,ids,context=context):
            if not line.visit_note:
                raise osv.except_osv(_('Error'), _('The visit note are mandatory on Route:%s Line:%s!'%(line.route_id.name,line.sequence)))
