    ...
    def write(self, cr, uid, ids, vals, context=None):
        ...
        if 'delivered' in vals.keys():
            for o in self.browse(cr, uid, isinstance(ids,int) and [ids] or ids, context=context):
                if o.route_line_id and o.route_line_id.id != context.get('route_line.id',0):
                    raise osv.except_osv(_('Invalid action !'), 
                            _('Cannot update a Picking(%s) which are already delivery routed (%s) !'%(o.name,o.route_id.name)))
