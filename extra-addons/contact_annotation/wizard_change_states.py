...
class contact_state_range(osv.osv):
    ...
    def _annotation_for_state_range(self, cr, uid, ids, field_name, args, context=None):
        ...
        for item in self.browse(cr, uid, ids, context=context):
            ...
            anno_ids=self.pool.get('contact.annotation').search(cr, uid, [('anno_type','in',anno_type_ids),('valid_to','>',item.datetime_from),('valid_from','<',item.datetime_to)], order='creation_date desc')
...
WHERE ats.state_id=%s AND ats.op='f' AND pf.id IS NOT NULL
    ...
    _columns={
        ...
        'domain_anno_types': fields.function(_domain_anno_types, type='one2many', method=True, obj='contact.type_annotation', ),
    ...
    def _default_range_states2(self, cr, uid, context=None):
        ...
        if context.get('client','')=='web':
            return context.get('web_active_ids',[])
        ...
        for planning in self.browse(cr,uid,ids,context=context):
            ...
            if not planning.anno_type:
                ...
                for (contact_id,range) in data.items():
                    self.pool.get('contact.annotation').create(cr, uid, {'contact_id': contact_id, 'informer': planning.user_id.id ,'anno_type': planning.anno_type.id, 'valid_from': range['from'], 'valid_to': range['to'], 'comment': planning.comment, 'creation_date': report_date})
        return True
