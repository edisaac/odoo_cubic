...
class account_invoice(osv.osv):
    ...
    def _spot(self, cr, uid, ids, field_name, arg, context=None):
        voucher_obj = self.pool.get('account.voucher')
        res = {}
        for invoice in self.browse(cr, uid, ids, context=context):
            number = ''
            date = None
            for ml in invoice.payment_ids:
                if ml.journal_id.spot_operation_type:
                    voucher_ids = voucher_obj.search(cr, uid, [('move_id','=',ml.move_id.id)], context=context)
    ...
    _columns = {
        'spot_operation_type' : fields.selection(lambda s,c,u,context={}:s.pool.get('base.element').get_selection(c,u,'PE.SPOT.ANEXO_05',context=c),
                                     'SPOT Operation Type',help="kind of operation subjected to SPOT (detractions) - SUNAT"),
        'spot_number' : fields.function(_spot, type="string", string="Spot Number", multi="spot"),
        'spot_date' : fields.function(_spot, type="string", string="Spot Date", multi="spot"),
    }
    ...
    def reset_transfer_spot(self, cr, uid, invoice, spot_amount,context=None):
        ...
        if spot_amount:
            ...
            ctx.pop('type', False)
            res = transfer_obj.create(cr,uid,self.transfer_spot_get(cr, uid, invoice, spot_amount, context=ctx),context=ctx)
        return res
    ...
    def round_spot(self, cr, uid, amount, invoice, amount_total=False, context=None):
        if context is None:
            context = {}
        if not amount:
            return amount
        ...
        if invoice.currency_id.id == invoice.company_id.currency_id.id:
            if amount_total < element_obj.get_float(cr,uid,'PE.SPOT.LIMITES','LIMITE_MINIMO'):
                amount = 0.0
            else:
                amount = round(amount)
        else:
            cur_context = context.copy()
            cur_context['date'] = invoice.date_invoice or time.strftime('%Y-%m-%d')
            if cur_obj.compute(cr, uid, invoice.currency_id.id, invoice.company_id.currency_id.id, amount_total, context=cur_context) < element_obj.get_float(cr,uid,'PE.SPOT.LIMITES','LIMITE_MINIMO'):
                amount = 0.0
            else:
                amount_company = round(cur_obj.compute(cr, uid, invoice.currency_id.id, invoice.company_id.currency_id.id, amount, context=cur_context))
                amount = cur_obj.compute(cr, uid, invoice.company_id.currency_id.id, invoice.currency_id.id, amount_company, context=cur_context)
        return amount
    ...
    def compute_spot_amount(self, cr, uid, invoice, context=None):
        ...
        for line in invoice.invoice_line:
            ...
            _spot_percent = max(_spot_percent,spot_percent)
            ...
            amount_total += line.price_subtotal + tax_amount
        if _spot_percent:
            spot_amount = amount_total * _spot_percent
        return self.round_spot(cr, uid, spot_amount, invoice, amount_total, context=context)
