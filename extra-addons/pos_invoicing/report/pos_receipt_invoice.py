...
class order(report_sxw.rml_parse):
    ...
    def untax(self, order_id):
        sql = 'select (price_unit/1.18), qty from pos_order_line where order_id = %s'
    ...
    def discount(self, order_id):
        ...
        for line in res:
            if line[0] != 0:
                dsum = dsum +(line[2] * (line[0]*line[1]/1.18/100))
        return dsum
...
report_sxw.report_sxw('report.pos.receipt.invoice', 'pos.order', 'addons/pos_invoicing/report/pos_receipt_invoice.rml', parser=order, header=False)
